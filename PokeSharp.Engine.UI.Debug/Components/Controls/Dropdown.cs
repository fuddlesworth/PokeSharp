...
public class Dropdown : UIComponent
{
    ...
    {
        // Draw main button
        var backgroundColor = IsOpen || IsHovered() ? Theme.ButtonHover : Theme.ButtonNormal;
    ...
    private void DrawDropdownList(UIContext context)
    {
        ...
        for (int i = 0; i < visibleCount; i++)
        {
            ...
            if (isItemHovered && context.Input.IsMouseButtonReleased(Input.MouseButton.Left))
            {
                SelectedIndex = i;
                IsOpen = false;
        ...
        if (!Rect.Contains(context.Input.MousePosition) &&
            ...
            context.Input.IsMouseButtonReleased(Input.MouseButton.Left))
        {
            IsOpen = false;
        }
    }
