...
public class HelpCommand : IConsoleCommand
{
    ...
    {
        string topic = args.Length > 0 ? args[0].ToLower() : "overview";
        ...
        switch (topic)
        {
            ...
            default:
                ...
                if (!ShowCommandHelp(context, topic))
                {
                    ...
                    context.WriteLine(
                        "Available topics: keyboard, commands, scripting",
                        context.Theme.TextSecondary
                    );
                    context.WriteLine(
                        "Or use 'help <command>' for command-specific help",
                        context.Theme.TextSecondary
                    );
                }
    ...
    private void ShowCommandsHelp(IConsoleContext context)
    {
        ...
        context.WriteLine(
            "Console commands help you control the debug environment.",
            theme.TextPrimary
        );
        context.WriteLine(
            "Commands can be executed with or without the '/' prefix.",
            theme.TextSecondary
        );
        context.WriteLine("");
        ...
        context.WriteLine("");
        context.WriteLine(
            "TIP: Type a command name and press Tab for auto-completion!",
            theme.Success
        );
    }
    ...
    private void ShowScriptingHelp(IConsoleContext context)
    {
        ...
        context.WriteLine(
            "The console supports full C# scripting with real-time evaluation.",
            theme.Success
        );
        ...
        context.WriteLine(
            "    return $\"Old: {money}, New: {Player.GetMoney()}\";",
            theme.SyntaxString
        );
        ...
        context.WriteLine(
            "    Player.AddMoney(1000)              Add 1000 to wallet",
            theme.SyntaxString
        );
        context.WriteLine(
            "    Math.Max(10, 20)                   Math functions",
            theme.SyntaxString
        );
        context.WriteLine(
            "    DateTime.Now                       System types",
            theme.SyntaxString
        );
        context.WriteLine(
            "    Enumerable.Range(1,10).Sum()       LINQ queries",
            theme.SyntaxString
        );
        ...
        context.WriteLine("");
        context.WriteLine(
            "TIP: Have fun experimenting! The console is your playground.",
            theme.Success
        );
    }
    ...
    {
        IConsoleCommand? command = context.GetCommand(commandName);
        if (command == null)
        {
            return false;
        }
        ...
        context.WriteLine(
            "Type 'help commands' to see all available commands.",
            theme.TextSecondary
        );
