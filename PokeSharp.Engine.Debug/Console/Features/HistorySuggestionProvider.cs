...
public class HistorySuggestionProvider
{
    ...
    )
    {
        ...
        var allHistory = _history.GetAll();
        if (allHistory.Count == 0)
            return new List<HistorySuggestion>();

        // Filter and rank commands
        var matches = new List<(string command, double score, int useCount)>();
        var commandFrequency = CountCommandFrequency(allHistory.ToList());

        foreach (var command in allHistory.Distinct())
        {
            ...
            var matchScore = CalculateMatchScore(command, currentInput);
            if (matchScore > 0)
            {
    ...
    {
        var index = command.IndexOf(input, StringComparison.Ordinal);
        if (index <= 0)
            return true;

        // Check if previous character is a word boundary (space, dot, parenthesis, etc.)
        var prevChar = command[index - 1];
    ...
    public List<HistorySuggestion> GetMostFrequentCommands(int maxResults = 10)
    {
        ...
        var frequency = CountCommandFrequency(allHistory.ToList());
