...
public class ConsoleInputRenderer
{
    ...
    )
    {
        ...
        int inputX;
        var inputY = y + ConsoleConstants.Rendering.InputYOffset;

        // Different layout for single-line vs multi-line
        if (input.IsMultiLine)
        {
            // Multi-line: Use line numbers instead of ">" prompt
            var maxLineNumber = input.LineCount;
            var digits = maxLineNumber.ToString().Length;
            var lineNumberWidth =
                (digits + ConsoleConstants.Rendering.LineNumberSuffixChars)
        ...
        else
        {
            ...
            _fontRenderer.DrawString(
                ">",
                Padding + ConsoleConstants.Rendering.GeneralSpacing,
                inputY,
                Prompt
            );
    ...
    )
    {
        ...
        var textBeforeSelection = text.Substring(0, selStart);
        var selectedText = text.Substring(selStart, selEnd - selStart);
        ...
        var currentLine = linesBeforeSelection.Length - 1;
        var startX = inputX;
        ...
        var startY = inputY + currentLine * lineHeight;
        ...
        for (var i = 0; i < selectedLines.Length; i++)
        {
            ...
            var rectY = startY + i * lineHeight;
            var rectHeight = lineHeight;

            // First line uses calculated startX, subsequent lines start at inputX
            var rectX = i == 0 ? startX : inputX;
    ...
    /// </summary>
    private void DrawBracketMatchingHighlight(
        int inputX,
        int inputY,
        int lineHeight,
        ConsoleInputField input
    )
    {
        ...
        var bracketPos = input.GetBracketNearCursor();
        if (bracketPos == -1)
            return;

        var matchingPos = input.FindMatchingBracket(bracketPos);
    ...
    /// </summary>
    private void HighlightCharacterAt(
        int position,
        int inputX,
        int inputY,
        int lineHeight,
    ...
    )
    {
        ...
        var highlightX = inputX + (int)offsetSize.X;
        var highlightY = inputY + line * lineHeight;
    ...
    private void DrawCursor(int x, int y, int lineHeight, ConsoleInputField input)
    {
        ...
        var cursorLine = input.GetCursorLine();
        var cursorColumn = input.GetCursorColumn();

        // Get the text up to the cursor position on the current line to measure width
        var textBeforeCursor = GetTextOnLine(cursorLine, input)
            .Substring(0, Math.Min(cursorColumn, GetTextOnLine(cursorLine, input).Length));
        var size = _fontRenderer.MeasureString(textBeforeCursor);

        var cursorX = x + (int)size.X;
        var cursorY = y + cursorLine * lineHeight;
