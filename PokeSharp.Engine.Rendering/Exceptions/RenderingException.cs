...
public class AssetLoadException : RenderingException
{
    ...
    public string AssetType =>
        Context.TryGetValue("AssetType", out var type) ? type?.ToString() ?? "" : "";
...
public class TextureLoadException : RenderingException
{
    ...
    public string TextureId =>
        Context.TryGetValue("TextureId", out var id) ? id?.ToString() ?? "" : "";
    public string FilePath =>
        Context.TryGetValue("FilePath", out var path) ? path?.ToString() ?? "" : "";
...
public class SpriteLoadException : RenderingException
{
    ...
    public string SpriteId =>
        Context.TryGetValue("SpriteId", out var id) ? id?.ToString() ?? "" : "";
...
public class CacheEvictionException : RenderingException
{
    ...
    public long CurrentCacheSize =>
        Context.TryGetValue("CurrentCacheSize", out var size) && size is long l ? l : 0L;
    public long MaxCacheSize =>
        Context.TryGetValue("MaxCacheSize", out var max) && max is long m ? m : 0L;
...
public class AnimationException : RenderingException
{
    ...
    public string AnimationId =>
        Context.TryGetValue("AnimationId", out var id) ? id?.ToString() ?? "" : "";
